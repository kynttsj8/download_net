...
func handleConnection(c net.Conn) {
	...
	// Receive client request for file download
	fileRequest, err := bufio.NewReader(c).ReadString('\n')
	if err != nil {
		fmt.Println(err)
		return
	}
	fileRequest = strings.TrimSpace(fileRequest)

	// Check if the requested file exists
	file, err := os.Open(fileRequest)
	if err != nil {
		fmt.Fprintf(c, "File not found\n")
		return
	}
	defer file.Close()
	...
	fmt.Fprintf(c, fileContent.String())
}
